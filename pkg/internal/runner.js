 ...
 */
const EmscriptenModuleFactory = (module, exports, requireFunc) => {
...
async function run() {
	try {
		// Read args from process.argv passed by Go
		// Usage: node runner.js <wasmPath> <requestPath> <body> <accessToken>
		var [, , wasmPath, reqPath, body, accessToken] = process.argv;

		reqPath = "/" + reqPath.replace(/^\/+/, ""); // Ensure leading slash

		console.error("DEBUG: Received reqPath:", reqPath);
		...
		const { SecureKey, Hmac } = instance;

		const keyStr =
			"wODdrvWqmdP4Zliay-iF3cz3KZcK0ekrial868apg06TXeCo7A1hIQO0ESElHg6D"; // Key for the extension
		const clientVersion = "3.7.1"; // Current client version

		// Hashing Helper
		const calculateSha256 = async (data) => {
			if (typeof data === "string") data = new TextEncoder().encode(data);
			return new Uint8Array(
				await global.window.crypto.subtle.digest("SHA-256", data)
			);
		};
		...
		const accessTokenHash = await calculateSha256(accessToken || ""); // Handle empty token
		...
		// Output ONLY the signature to stdout for Go to capture
